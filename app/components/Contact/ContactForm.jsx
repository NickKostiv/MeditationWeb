...
export default function ContactForm() {
  ...
  const handleContinue = () => {
    Cookies.set(
      "contactFormData",
      JSON.stringify({ firstName, email, question }),
      { expires: 7 }
    );
    setIsSent(true);
    ...
    <div className="relative w-full h-screen overflow-hidden bg-[radial-gradient(77.28%_53.46%_at_80.97%_37.09%,rgb(12,38,18)_0%,rgb(8,1,19)_100%)] flex flex-col items-center md:justify-start text-white">
      <div className="px-[20px] w-full max-w-[550px] mt-[50px] rounded-md text-center">
        <div>
          ...
          />
          <h1
            className="text-[19px] md:text-[22px] uppercase font-extrabold font-sans my-[15px] sm:mt-[60px] sm:mb-[20px] text-center"
            style={{
              backgroundImage:
        ...
        <form className="flex flex-col items-center w-full mt-4 md:mt-[50px]">
          ...
          {isEmailValid && (
            <div data-aos="fade-up" data-aos-delay="400" className="w-full">
              <div className="flex mt-[20px] relative mb-5 p-0 px-[10px] w-full items-center border-2 border-[hsla(0,0%,100%,0.67)] rounded-[17px] transition-all duration-[0.4s] ease-in-out">
                <textarea
                  name="question"
                  placeholder="Type your question"
                  className="w-full p-[15px] md:p-[20px] text-[16px] md:text-[20px] bg-transparent text-white outline-none resize-none h-[120px]"
                  value={question}
        ...
        {/* Continue Button */}
        {isEmailValid && question.length > 0 && (
          <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-full max-w-[550px] bg-[#4646464d] z-30 text-center py-[16px] px-[20px] rounded-t-[30px]">
            <button
              className="relative w-full max-w-[550px] font-sans inline-flex mx-auto items-center justify-center bg-[#2fd1db] text-white text-[20px] md:text-[24px] font-semibold uppercase py-[7px] px-[20px] rounded-[64px] shadow-none transition-all duration-300 ease-in-out focus:outline-none active:scale-90 disabled:opacity-50"
